...
//
// Modified for akira:
//   - hintImage is no longer created; akira does not ship switchfin's img/empty.png, and
//     loading a missing resource left an invalid texture that was redrawn every frame.
//   - draw() renders the hint label on its own when there is no hint image.
//   - setEmpty()/setError() null-guard their setImageFromRes calls.
//
// This is vendored from switchfin under the Apache-2.0 license. Please check the attached
// license for the terms of the Apache license
//
...
RecyclingGrid::RecyclingGrid() {
    ...
    // Create hint views
    // akira does not ship switchfin's hint art; the hint is text-only and
    // hintImage stays null. Every use below is null-guarded.
...
void RecyclingGrid::setEmpty(std::string msg) {
    if (this->hintImage) this->hintImage->setImageFromRes("img/empty.png");
    this->hintLabel->setText(msg);
    this->clearData();
}

void RecyclingGrid::setError(std::string error) {
    if (this->hintImage) this->hintImage->setImageFromRes("img/net_error.png");
